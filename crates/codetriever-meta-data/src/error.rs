//! Structured error handling for the metadata layer
//!
...
pub type DatabaseResult<T> = std::result::Result<T, MetaDataError>;

/// Legacy type alias for backwards compatibility
pub type DatabaseError = MetaDataError;
...
/// Comprehensive metadata error with full context
#[derive(Debug, Error)]
pub enum MetaDataError {
    ...
    BatchOperationFailed {
        ...
        correlation_id: Option<String>,
        #[source]
        source: Option<Box<MetaDataError>>,
    },
...
pub trait MetaDataErrorExt<T> {
    /// Convert to `MetaDataError` with operation context
    ///
    /// # Errors
    /// Returns `MetaDataError` with operation context and correlation ID
    fn map_db_err(
        self,
        operation: DatabaseOperation,
        correlation_id: Option<String>,
    ...
    /// Convert to `MetaDataError` with operation context and custom error mapping
    ///
    /// # Errors
    /// Returns `MetaDataError` from custom mapping function
    ...
    where
        F: FnOnce(sqlx::Error) -> MetaDataError;
}
...
impl<T> MetaDataErrorExt<T> for std::result::Result<T, sqlx::Error> {
    fn map_db_err(
        ...
        self.map_err(|e| MetaDataError::query_failed(operation, e, correlation_id))
    }
...
impl<T> DatabaseErrorExt<T> for std::result::Result<T, sqlx::Error> {
    fn map_db_err(
        ...
        MetaDataErrorExt::map_db_err(self, operation, correlation_id)
    }
    ...
    where
        F: FnOnce(sqlx::Error) -> MetaDataError,
    {
        MetaDataErrorExt::map_db_err_with(self, operation, correlation_id, f)
    }
}
...
mod tests {
    ...
    fn test_correlation_id_propagation() {
        let error = MetaDataError::pool_exhausted(ConnectionPoolType::Write, 10, 30, None);
