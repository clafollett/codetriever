...
const DEFAULT_CONCURRENCY_LIMIT: usize = 4; // Reasonable parallelism
// NO OVERLAP - rely on semantic boundaries (functions/classes) + byte offsets for context
...
impl IndexingConfig {
    ...
    pub fn from_env() -> Self {
        ...
        let split_large_units = std::env::var("CODETRIEVER_INDEXING_SPLIT_LARGE_UNITS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(DEFAULT_SPLIT_LARGE_UNITS);
...
mod tests {
    ...
    fn test_application_config_can_be_created() {
        ...
        assert_eq!(config.embedding.model.max_tokens, DEFAULT_MAX_TOKENS);
    }
    ...
    fn test_config_uses_safe_defaults() {
        ...
        assert_eq!(config.embedding.model.max_tokens, DEFAULT_MAX_TOKENS);
        assert_eq!(config.indexing.concurrency_limit, DEFAULT_CONCURRENCY_LIMIT);
    ...
    fn test_embedding_model_consistency() {
        ...
        assert_eq!(config.embedding.model.dimensions, DEFAULT_DIMENSIONS);
    }
