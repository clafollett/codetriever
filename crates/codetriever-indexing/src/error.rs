...
impl From<codetriever_meta_data::MetaDataError> for IndexerError {
    fn from(err: codetriever_meta_data::MetaDataError) -> Self {
        Self::Other(format!("Database error: {err}"))
    }
}

impl From<codetriever_embeddings::EmbeddingError> for IndexerError {
    fn from(err: codetriever_embeddings::EmbeddingError) -> Self {
        Self::Other(format!("Embedding error: {err}"))
    }
}

impl From<codetriever_vector_data::VectorDataError> for IndexerError {
    fn from(err: codetriever_vector_data::VectorDataError) -> Self {
        Self::Other(format!("Vector data error: {err}"))
    }
}

impl From<codetriever_parsing::ParsingError> for IndexerError {
    fn from(err: codetriever_parsing::ParsingError) -> Self {
        Self::Other(format!("Parsing error: {err}"))
    }
}
