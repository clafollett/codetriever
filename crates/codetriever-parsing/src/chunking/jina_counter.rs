...
impl JinaTokenCounter {
    ...
    /// Load tokenizer from file path (for testing/demo purposes)
    /// In production, this would integrate with the embedding model crate
    pub async fn from_tokenizer_file(
        tokenizer_path: &str,
        max_tokens: usize,
    ) -> ParsingResult<Self> {
        let tokenizer = Tokenizer::from_file(tokenizer_path)
            .map_err(|e| crate::ParsingError::Other(format!("Failed to load tokenizer: {e}")))?;
